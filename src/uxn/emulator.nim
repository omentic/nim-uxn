...
func step*(program: var Program) =
  ...
  of JMP:
    if program.opcode.short():
      ...
      if relative >= 0: program.pc += uint8(relative.abs)
      else: program.pc -= uint8(relative.abs)
  of JCN:
    if program.opcode.short():
      ...
      if condition != 0:
        ...
        if address >= 0: program.pc += uint8(address.abs)
        else: program.pc -= uint8(address.abs)
  of JSR:
  ...
  of LDA: program.handle((a, b) => (program.main.get(uint16(a, b))))
  of STA: program.handle((a, b, c) => (program.main.set(uint16(b, c), a)))
